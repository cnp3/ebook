.. Copyright |copy| 2010, 2019 by Olivier Bonaventure et al.
...
The main objective of the network layer is to allow end systems, connected to different networks, to exchange information through intermediate systems called :term:`router`. The unit of information in the network layer is called a :term:`packet`.
...
Several techniques to update the forwarding tables upon the arrival of a packet have been used in deployed networks. In this section, we briefly present the principles that underlie three of these techniques.
...
.. tikz:: A simple heterogeneous network
      ...
      \node[router, right=of A] (R1) {R1};
      \node[router, right=of R1] (R2) {R2};
      \node[host, right=of R2] (B) {B};
      \draw[black] (A) -- (R1) node [midway] {Max 1000 bytes};
      \draw[black] (R1) -- (R2) node [midway] {Max 500 bytes};
      \draw[black] (R2) -- (B) node [midway] {Max 1000 bytes}; 
